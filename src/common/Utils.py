...
def performed_tasks_naive(locs, max_dist, t, FCFS, seed):
    ...
    if FCFS:
        ...
        for i in purmuted_locs:
            performed, dist = performed_task(locs[i], max_dist, t, seed)
            if performed:
                return True, dist
    else:
        locs = sorted(locs, key = lambda loc: distance(loc[0], loc[1], t[0], t[1]))
        for loc in locs:
            performed, dist = performed_task(loc, max_dist, t, seed)
            if performed:
                return True, dist
...
def is_range_overlap(range1, range2):
    ...
    """
    if range2[0]<=range1[0]<=range2[1] or range2[0]<=range1[1]<=range2[1] or range1[0]<=range2[0]<=range1[1] or range1[0]<=range2[1]<=range1[1]:
        return True 
