...
#    Params.maxHeight = 10
    ...
    np.savetxt(Params.resdir+exp_name+'_abs_'+str(int(queryShape[0]*10))+'_'+str(int(queryShape[1]*10)), res_abs_summary, fmt='%.4f')
    np.savetxt(Params.resdir+exp_name+'_rel_'+str(int(queryShape[0]*10))+'_'+str(int(queryShape[1]*10)), res_rel_summary, fmt='%.4f')
    ...
    for eps in epsList:
        for type in ['abs','rel']:
            ...
            for queryShape in q_list:
                fileName = Params.resdir + exp_name + '_' + type + '_' + str(int(queryShape[0]*10)) + '_' + str(int(queryShape[1]*10))
                try:
