...
class Kd_pure(KTree):
    ...
    def getSplitBudget(self):
        ...
        return [0 for _ in range(Params.maxHeight)]

    def getCountBudget(self):
        """ zero count budget mean exact node count """
        return [0 for _ in range(Params.maxHeight + 1)]
    ...
    def getCoordinates(self, curr):
        ...
        _data[:, :] = _data[:, _idx]
        splitP = self.getSplit(_data[dimP, :], _box[0, dimP], _box[1, dimP],
                               budget_s[curr.n_depth] / 2)  # ## get split point for dim_1
