...
class HT_pure(HTree):
    ...
    def __init__(self, data, param):
        ...
        if param.dynamicGranularity == True:
            count_eps = self.param.Eps*(1-self.param.PercentSplit)
            self.param.partitionsHTree[0] = self.param.partitionsHTree[1] = int(math.ceil((Params.NDATA*count_eps/Params.c_htree)**(1.0/2)))
            self.param.switchPartitionsHTree[0] = self.param.switchPartitionsHTree[1] = 2**(int(math.ceil(math.log(self.param.partitionsHTree[0],2)/2.0)))
    ...
    def getSplitBudget(self, curr):
        ...
        return [0 for i in range(self.maxSplit[dimP])]
    
    # overrided in HT_true
    def getCountBudget(self):
        """ zero count budget mean exact node count """
        return [0 for i in range(self.param.maxHeightHTree+1)]
